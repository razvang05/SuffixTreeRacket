...
(define (get-ch-branch st ch)
 (cond
    ((st-empty? st) #f)
    ; extrag prima ramura apoi extrag si eticheta si ii verific
    ; primul caracter cu ch , in caz contrar apelez recursiv
    ; fara prima ramura pe care deja am prelucrat o
    ((equal? (stream-first (get-branch-label (first-branch st))) ch) (first-branch st))
